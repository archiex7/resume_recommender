...
class CvTemplate1:
    ...
    def job_title_parser(input_str):
        regex_ = r"職務名稱:(.*?)(?=⼯作內容)"  # TODO: be careful with font
...
class CvTemplate2:
    ...
    @staticmethod
    def work_content_parser(input_str):
        regex_ = r"薪資待遇:(.*?)(?=⼯作技能)"  # TODO: be careful with font
        work_contents = re.findall(regex_, input_str, re.DOTALL)
        ans = {
            'work_contents': work_contents,
            'length': len(work_contents)
...
class CvTemplate3:
    ...
    @staticmethod
    def work_content_parser(input_str):
        regex_ = r"離職原因:(.*?)(?=⼯作技能)"  # TODO: be careful with font
        work_contents = re.findall(regex_, input_str, re.DOTALL)
        ans = {
            'work_contents': work_contents,
            'length': len(work_contents)
